    ...
    <div class="root-fb">
        <div class="toolbar">
            ...
            <div class="tb-btn-group">
                ...
                <span class="material-symbols-rounded" id="tb-btn" onclick="document.getElementById('newGoal').showModal()">add</span>
            </div>
        </div>
        <div class="flexbox">
            <div class="col1" id="ca">
                <div class="content-area">
                    ...
                    $total_query = "SELECT COUNT(*) as total FROM Goal";
                    $total_result = $db->query($total_query);
                    $total_row = $total_result->fetchArray(SQLITE3_ASSOC);
                    $total_records = $total_row['total'];
                    $total_pages = ceil($total_records / $records_per_page);

                    $select_query = "SELECT * FROM Goal LIMIT $offset, $records_per_page";
                    ...
                    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
                        $bookingID = $row['goalID'];
                        echo '<tr><td>' . $row['goalID'] . '</td>' . '<td>' . $row['name'] . '</td>' . '<td>' . $row['amount'] . '</td>' . '<td>' . $row['targetDate'] . '</td> <td><a style="cursor: pointer;" onclick="predict('.$bookingID.')"><span class="material-symbols-rounded">online_prediction</span></a></td><td><a style="cursor: pointer;" onclick="raiseUpdate('.$bookingID.')"><span class="material-symbols-rounded">edit</span></a></td> <td><a style="cursor: pointer;" onclick="deleteDialog('.$bookingID.')"><span class="material-symbols-rounded">delete</span></a></td> </tr>';
                        echo '<script type="text/JavaScript">cats.push("'.$row['name'].'")</script>';
                        echo '<script type="text/JavaScript">totals.push("'.$row['amount'].'")</script>';
                    }
                    echo '</tbody>';
                    echo '</table>';
...
<script>
    ...
    function nextPage(){
        document.getElementById('newGoal-pg1').style.display = 'none';
        document.getElementById('newGoal-pg2').style.display = 'flex';
    }
    function completeDialog(){
        document.getElementById('newGoal').close();
        location.reload();
    }
    function completeDeleteDialog(){
        document.getElementById('delGoal').close();
        document.getElementById('delGoal').style.removeProperty('display');
        location.reload();
    }
    function deleteDialog(delpayid){
        document.getElementById('delGoal').showModal();
        document.getElementById('delGoal').style.display = 'flex';
        document.getElementById('delOutputArea').src = "goal-delete.php?goalID="+delpayid;
    }
    function raiseUpdate(updpayid){
        document.getElementById('updGoal').showModal();
        document.getElementById('updGoal').style.display = 'flex';
        document.getElementById('updOutputArea').src = "updatef-goal.php?goalID="+updpayid;
    }
    ...
    function finishUpdate(){
        ...
        document.getElementById('updGoal').close();
        document.getElementById('updGoal').style.display = 'none';
        location.reload();
        cancelAnimationFrame(handoffHandler);
    }
    function predict(goalID){
        document.getElementById('predGoal').showModal();
        document.getElementById('predGoal').style.display = 'flex';
        document.getElementById('predOutputArea').src = "predict-goal.php?goalID="+goalID;
        document.getElementById('pred-done-btn').style.removeProperty('display');
    }
    function finishPredict(){
        document.getElementById('pred-close').style.display = 'none';
        document.getElementById('predGoal').close();
        document.getElementById('predGoal').style.display = 'none';
        document.getElementById('pred-done-btn').style.display = 'none';
    }
</script>
<dialog id="newGoal">
    <div id="newGoal-pg1">
        <span class="material-symbols-rounded" style="cursor: pointer;" onclick="document.getElementById('newGoal').close()">close</span>
        <div class="fieldset">
            <form action="goal-new.php" method="post" autocomplete="on" target="outputArea" onsubmit="nextPage()">
    ...
    <div id="newGoal-pg2" style="display: none;flex-direction: column;">
        <iframe name="outputArea"></iframe>
        <button onclick="completeDialog()" style="margin-top:5px;">Done</button>
    </div>
</dialog>
<dialog id="delGoal" style="display: none;flex-direction: column;">
    <iframe id="delOutputArea"></iframe>
    <button onclick="completeDeleteDialog()" style="margin-top:5px;">Done</button>
</dialog>
<dialog id="updGoal" style="display: none;flex-direction: column">
    <span class="material-symbols-rounded" style="cursor: pointer;" onclick="document.getElementById('updGoal').close();document.getElementById('updGoal').style.display = 'none';" id="upd-close">close</span>
...
<dialog id="predGoal" style="display: none;flex-direction: column">
    <span class="material-symbols-rounded" style="cursor: pointer;" onclick="document.getElementById('predGoal').close();document.getElementById('predGoal').style.display = 'none';" id="pred-close">close</span>
    <iframe id="predOutputArea" style="height: 200px;width:360px;"></iframe>
    <button onclick="finishPredict()" style="margin-top:5px;display: none;" id="pred-done-btn">Done</button>
</dialog>
</body>
