...
<div class="root-split" style="display: flex; flex-direction: row;width: 100%;">
    ...
    <div class="root-fb">
        <div class="toolbar">
            <h1 class="list-header">Savings Accounts</h1>
        ...
        <div class="flexbox">
            <div class="col1" id="ca">
                <div class="content-area">
                    ...
                    $total_query = "SELECT COUNT(*) as total FROM Account WHERE type = 'Savings'";
                    $total_result = $db->query($total_query);
                    $total_row = $total_result->fetchArray(SQLITE3_ASSOC);
                    $total_records = $total_row['total'];
                    $total_pages = ceil($total_records / $records_per_page);

                    $select_query = "SELECT * FROM Account WHERE type = 'Savings' LIMIT $offset, $records_per_page";
                    ...
                    while ($row = $result->fetchArray(SQLITE3_ASSOC)) {
                        ...
                        echo '<tr><td>' . $row['accountID'] . '</td>' . '<td>' . $row['name'] . '</td>' . '<td>' . $row['interestRate'] . '</td>' . '<td>' . $row['balance'] . '</td> <td><a style="cursor: pointer;" onclick="predict('.$bookingID.')"><span class="material-symbols-rounded">online_prediction</span></a></td><td><a style="cursor: pointer;" onclick="raiseUpdate('.$bookingID.')"><span class="material-symbols-rounded">edit</span></a></td> <td><a style="cursor: pointer;" onclick="deleteDialog('.$bookingID.')"><span class="material-symbols-rounded">delete</span></a></td> </tr>';
            ...
            <div class="col2" id="col2">
                <div class="subpane">
                    <div class="pane-header">
                        <h3>Savings Accounts by Balance</h3>
...
<script>
    ...
    function predict(accID) {
        document.getElementById('predictor').showModal();
        document.getElementById('predictor').style.display = 'flex';
        document.getElementById('predOutputArea').src = "predict-savings.php?accountID="+accID;
    }
    function finishPredict(){
        document.getElementById('predictor').close();
...
<dialog id="predictor" style="display: none;flex-direction: column">
    <iframe id="predOutputArea" style="width: 400px; height: 125px;"></iframe>
    <button onclick="finishPredict()" style="margin-top:5px;">Done</button>
</dialog>
</body>